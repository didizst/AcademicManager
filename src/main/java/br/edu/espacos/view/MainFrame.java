// /main/java/br/edu/espacos/view/MainFrame.java
package br.edu.espacos.view;

import br.edu.espacos.App;
...
public class MainFrame extends JFrame {
    ...
    // Adicione um campo para o JFrame pai
    private JFrame parentFrame;

    // Modifique o construtor para aceitar o parentFrame
    public MainFrame(JFrame parentFrame, EspacosClient client, String nomeUsuario, String tipoUsuario, String usuarioId) {
        this.parentFrame = parentFrame; // Armazena a referência ao frame pai
        this.client = client;
    ...
    private void deslogar() {
        ...
        if (confirmacao == JOptionPane.YES_OPTION) {
            ...
            this.dispose(); // Libera os recursos da janela principal

            // Notifica o frame pai (App) para reabrir a tela de login
            if (parentFrame instanceof App) { // Verifica se o parentFrame é uma instância de App
                ((App) parentFrame).reopenLogin(); // Chama o método para reabrir o login
            } else {
                System.exit(0); // Fallback caso não seja chamado de App
            }
        }
    }
}
