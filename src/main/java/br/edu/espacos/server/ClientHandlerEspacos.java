...
import br.edu.espacos.util.CustomLogger; // Import adicionado
...
public class ClientHandlerEspacos implements Runnable {
    ...
    public void run() {
        try {
            ...
            CustomLogger.log("Cliente conectado: " + clientSocket.getInetAddress().getHostAddress()); // Log de conexão
            ...
            CustomLogger.log("Erro de comunicação com o cliente " + (usuarioLogado != null ? usuarioLogado : clientSocket.getInetAddress().getHostAddress()) + ": " + e.getMessage()); // Log de erro de comunicação
        } finally {
            try {
                if (clientSocket != null && !clientSocket.isClosed()) {
                    clientSocket.close();
                    CustomLogger.log("Cliente desconectado: " + (usuarioLogado != null ? usuarioLogado : clientSocket.getInetAddress().getHostAddress())); // Log de desconexão
                }
            } catch (IOException e) {
                CustomLogger.log("Erro ao fechar socket do cliente: " + e.getMessage()); // Log de erro ao fechar socket
            }
        }
    }
        ...
        try {
            switch (acao) {
                ...
                case "GERAR_RELATORIO":
                    ...
                    break;
                case "LOGOUT":
                    CustomLogger.log("Usuário " + usuarioLogado + " realizou logout."); // Log de logout
                    usuarioLogado = null;
                ...
                default:
                    ...
                    CustomLogger.log("Comando não reconhecido de " + (usuarioLogado != null ? usuarioLogado : clientSocket.getInetAddress().getHostAddress()) + ": " + acao); // Log de comando não reconhecido
            }
        } catch (Exception e) {
            enviarMensagem("ERRO|Erro interno do servidor: " + e.getMessage());
            CustomLogger.log("Erro interno do servidor ao processar comando '" + acao + "' para " + (usuarioLogado != null ? usuarioLogado : clientSocket.getInetAddress().getHostAddress()) + ": " + e.getMessage()); // Log de erro interno
        }
    }
    ...
    private void processarLogin(String[] partes) throws IOException {
        ...
        if (usuario != null && usuario.getSenha().equals(senha) && usuario.isAtivo()) {
            ...
            CustomLogger.log("Login bem-sucedido para o usuário: " + email + " (ID: " + usuario.getId() + ")"); // Log de login bem-sucedido
        } else {
    ...
    private void processarRegistro(String[] partes) throws IOException {
        if (partes.length < 5) {
            ...
            CustomLogger.log("Tentativa de registro com formato inválido."); // Log de formato inválido
            return;
        }
        ...
        if (UsuarioStorage.emailJaExiste(email)) {
            ...
            CustomLogger.log("Tentativa de registro falhou: email '" + email + "' já existe."); // Log de email existente
            return;
        }
        ...
        try {
            ...
            CustomLogger.log("Novo usuário registrado: " + nome + " (Email: " + email + ", Tipo: " + tipo.name() + ", ID: " + novoId + ")"); // Log de novo registro
        } catch (IllegalArgumentException e) {
            enviarMensagem("REGISTRO_ERRO|Tipo de usuário inválido");
            CustomLogger.log("Tentativa de registro falhou: tipo de usuário inválido '" + tipoStr + "'."); // Log de tipo de usuário inválido
        }
    }
    ...
    private void exportarTodasReservas() throws IOException {
        ...
        CustomLogger.log("Administrador " + usuarioLogado + " exportou todas as reservas."); // Log de exportação
    }
    ...
    private void cadastrarEspaco(String[] partes) throws IOException {
        ...
        try {
            ...
            CustomLogger.log("Administrador " + usuarioLogado + " cadastrou novo espaço: " + nome + " (ID: " + novoId + ", Tipo: " + tipo.name() + ")"); // Log de cadastro de espaço
    ...
    private void removerEspaco(String[] partes) throws IOException {
        ...
        CustomLogger.log("Administrador " + usuarioLogado + " removeu o espaço: " + espaco.getNome() + " (ID: " + espacoId + ")."); // Log de remoção de espaço
    }
    ...
    private void fazerReserva(String[] partes) throws IOException {
        ...
        try {
            ...
            if (usuario.getTipo() == TipoUsuario.ADMIN) {
                ...
                CustomLogger.log("Administrador " + usuarioLogado + " fez uma reserva (CONFIRMADA) para o espaço " + espaco.getNome() + " (ID: " + espacoId + ") de " + dataHoraInicioStr + " a " + dataHoraFimStr + "."); // Log de reserva de admin
            } else {
                novaReserva = new Reserva(novoId, espacoId, usuarioLogado, dataHoraInicio, dataHoraFim, finalidade, observacoes);
                CustomLogger.log("Usuário " + usuarioLogado + " fez uma reserva (PENDENTE) para o espaço " + espaco.getNome() + " (ID: " + espacoId + ") de " + dataHoraInicioStr + " a " + dataHoraFimStr + "."); // Log de reserva de usuário comum
            }
            ...
            CustomLogger.log("Reserva ID " + novaReserva.getId() + " realizada com sucesso pelo usuário " + usuarioLogado + " para o espaço " + espaco.getNome() + "."); // Log de reserva bem-sucedida
        } catch (Exception e) {
    ...
    private void cancelarReserva(String[] partes) throws IOException {
        ...
        CustomLogger.log("Reserva ID " + reservaId + " cancelada com sucesso pelo usuário " + usuarioLogado + "."); // Log de cancelamento de reserva
    }
    ...
    private void listarReservas() throws IOException {
        ...
        enviarMensagem(sb.toString());
        CustomLogger.log("Administrador " + usuarioLogado + " listou todas as reservas."); // Log de listagem de todas as reservas
    }
    ...
    private void deletarUsuario(String[] partes) throws IOException {
        ...
        CustomLogger.log("Administrador " + usuarioLogado + " desativou o usuário: " + usuarioADeletar.getNome() + " (ID: " + usuarioId + ")."); // Log de desativação de usuário
    }

    private void alterarTipoUsuario(String[] partes) throws IOException {
        if (usuarioLogado == null) {
            enviarMensagem("ERRO|Usuário não logado");
            CustomLogger.log("Tentativa de alterar tipo de usuário sem estar logado."); // Log de acesso não autorizado
            return;
        }
        ...
        String tipoAntigo = usuarioAlvo.getTipo().name();
        ...
        CustomLogger.log("Administrador " + usuarioLogado + " alterou o tipo do usuário " + usuarioAlvo.getNome() + " (ID: " + usuarioId + ") de " + tipoAntigo + " para " + novoTipo.name() + "."); // Log de alteração de tipo de usuário
    }
    ...
    // Este método não é mais usado para logging principal, pois CustomLogger é mais centralizado
    private void salvarReservaEmArquivo(Reserva reserva) {
        String fileName = "reservas_log.txt"; // Este é um log separado, não o system_log.txt
